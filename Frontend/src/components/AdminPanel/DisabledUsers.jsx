...
const DisabledUsers = () => {
  ...
  const handleEnable = async (id) => {
    try {
      ...
      setUsers((prevUsers) =>
        prevUsers.map(
          (user) => (user._id === id ? { ...user, status: "active" } : user) // Update local state after enabling
        )
      );
    } catch (error) {
  ...
  const filteredUsers = users
    .filter((user) => user.status === "disabled") // Filter users with 'disabled' status
    .filter(
      (user) =>
  ...
  return (
    <div className="admin-content">
      <h2 className="admin-panel-heading">Disabled Users</h2>
